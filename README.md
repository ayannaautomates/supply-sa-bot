...
- **Certification type knowledge** (`certification-types-knowledge.js`) — distinguishes the 10 certifications Supply SA actually administers (SBE, ESBE, MBE, WBE, VBE, DIBE, NABE, AABE, ABE, HABE — all selectable in the readiness flow) from certifications Supply SA does NOT certify (DBE/TxDOT, ACDBE, WOSB, EDWOSB, VOSB, SDVOSB, 8(a)/all SBA federal programs, VetHUB/Texas Comptroller). When a vendor asks "do you do WOSB?" or "what's the difference between MBE and DBE?", the bot answers from this real distinction instead of guessing or implying Supply SA certifies something it doesn't.
