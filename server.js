...
function buildSystemPrompt(readinessReport, knowledgeContext, faqContext) {
  ...
  const knowledgeBlock = knowledgeContext
    ? `\n# Relevant Document Requirements\nUse this to explain WHY a document is needed or what it must contain — these are the real Supply SA certification rules. Speak in plain terms, not the internal rule format.\n${knowledgeContext}\n`
    : "";
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const response = await anthropic.messages.create({
      model: MODEL,
      max_tokens: 400,
      system: buildSystemPrompt(readinessReport, knowledgeContext, faqContext),
...
app.post("/api/readiness-flow/next-question", (req, res) => {
  ...
  // straight to the condensed renewal guidance instead of the full
  // profile + document walkthrough, per the SOP.
  if (answers.app_type === "renewal") {
    return res.json({
      done: true,
      isRenewal: true,
      renewalMessage: RENEWAL_GUIDANCE.message,
    });
  }
