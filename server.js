...
import { buildCertificationContext } from "./certification-types-knowledge.js";
...
function buildSystemPrompt(knowledgeContext, faqContext, guidanceContext, certificationContext) {
  ...
  const certificationBlock = certificationContext
    ? `\n# Relevant Certification Type Information\nUse this to accurately explain certification types the vendor asked about. CRITICAL: some certifications listed here are certified BY Supply SA, and others are NOT — administered by other agencies entirely (federal SBA, TxDOT, Texas Comptroller, airport authorities). Never imply Supply SA certifies something it doesn't. If a vendor asks about a certification Supply SA doesn't administer, clearly say so and point them to the correct agency.\n${certificationContext}\n`
    : "";
...
${guidanceBlock}${knowledgeBlock}${faqBlock}${certificationBlock}
# Handoff Signal
...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const certificationContext = buildCertificationContext(message);

    const response = await anthropic.messages.create({
      model: MODEL,
      max_tokens: 400,
      system: buildSystemPrompt(knowledgeContext, faqContext, guidanceContext, certificationContext),
